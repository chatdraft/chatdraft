...
Click this button to copy the default directory on Windows for Marvel Snap's
game state files:
<button
...
<input
  ...
  accept=".json" />
<br />
<button class="btn btn-md variant-filled-primary mt-2" on:click={Submit}
  >Upload Collection</button>
{#if !collectionComplete}
  <button
    class="btn btn-md variant-outline-warning"
    formaction="?/resetCollection">Reset to Collection Complete</button>
{/if}
