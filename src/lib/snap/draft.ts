...
export class Draft extends EventEmitter implements IDraft {
  ...
  public constructor(
    ...
    if (collection) {
      this.all_cards = all_cards.all.filter((card) =>
        collection.includes(card.cardDefKey)
      );
    }
  }
  cards: Deck = [];
  ...
  onVotingClosed =
    this.registerEvent<
      [player_channel: string, result: string, ties: string[]]
    >();
  ...
  onChoiceOverride =
    this.registerEvent<[player_channel: string, result: string]>();
  ...
  public async Choose(
    ...
    if (override && this.player != '')
      this.emit(this.onChoiceOverride, this.player, cardDefKey);
  ...
  private CanChoose(cardDefKey: string | undefined | null) {
    ...
    const validChoice =
      this.currentChoice &&
      this.currentChoice.cards.some((card) => card.cardDefKey == cardDefKey);
  ...
  public GetDeckCode(): string {
    ...
    this.cards.forEach((card) =>
      obj.Cards.push({ CardDefId: card.cardDefKey })
    );
