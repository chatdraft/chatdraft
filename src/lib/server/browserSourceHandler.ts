...
export function CloseBrowserSource(channel_name: string, websocket_id: string) {
  if (browserSourceStatuses.has(channel_name)) {
    browserSourceStatuses
      .get(channel_name)
      ?.full_sources.forEach((ws_id, index) => {
        if (ws_id == websocket_id) {
          browserSourceStatuses
            .get(channel_name)
            ?.full_sources.splice(index, 1);
        }
      });
    browserSourceStatuses
      .get(channel_name)
      ?.deck_sources.forEach((ws_id, index) => {
        if (ws_id == websocket_id) {
          browserSourceStatuses
            .get(channel_name)
            ?.deck_sources.splice(index, 1);
        }
      });
    browserSourceStatuses
      .get(channel_name)
      ?.choice_sources.forEach((ws_id, index) => {
        if (ws_id == websocket_id) {
          browserSourceStatuses
            .get(channel_name)
            ?.choice_sources.splice(index, 1);
        }
      });
...
export function RegisterFullBrowserSource(
  channel_name: string,
  websocket_id: string
) {
  if (!browserSourceStatuses.has(channel_name)) {
...
export function RegisterDeckBrowserSource(
  channel_name: string,
  websocket_id: string
) {
  if (!browserSourceStatuses.has(channel_name)) {
...
export function RegisterChoiceBrowserSource(
  channel_name: string,
  websocket_id: string
) {
  if (!browserSourceStatuses.has(channel_name)) {
