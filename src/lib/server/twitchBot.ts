...
export default class TwitchBot {
    ...
    this.bot = new Bot({
      ...
      commands: [
        createBotCommand(
          ...
          async (_, { say }) =>
            await say(
              `Help  draft a deck and I'll play it! A random choice of cards will be presented and chat will vote on which card gets added to the deck. Type the number to vote! To use Chat Draft, inquire at twitch.tv/jjrolk`
            )
        ),
        createBotCommand(
          'chatdraftstart',
          async (params, { broadcasterName, msg }) => {
            if (msg.userInfo.isMod || msg.userInfo.isBroadcaster) {
              ...
              const collection = preferences?.collection
                ? JSON.parse(preferences?.collection)
                : null;
  ...
  public static async NewChoice(player_channel: string, choice: Choice) {
    let expression = 'Vote ';
    choice.cards.forEach(
      (card, index) => (expression += `(${index + 1}) ${card.name} `)
    );
  ...
  public static async DraftComplete(player_channel: string, deck: Deck) {
    TwitchBot.Say(
      player_channel,
      `The completed deck has been drafted: ${deck
        .map((card) => card.name)
        .join(', ')}.`
    );
  }
  ...
  public static async VotingClosed(
    ...
    if (ties.length > 1) {
      TwitchBot.Action(
        player_channel,
        `${result} chosen after tie between ${ties.join(', ')}.`
      );
    } else {
