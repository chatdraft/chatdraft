<script lang="ts">
  ...
  const handleMessage = async (message: string) => {
    ...
    if (message == 'showdeck') {
      ...
      const ret = await fetch(
        `/api/v1/draft/player/${data.player}?previous=true`
      );
      if (!ret.ok) return;
  ...
  $: time_remaining =
    (current_draft?.currentChoice?.votes_closed! - now) / 1000;
...
{#if current_draft}
  {#if time_remaining}
    <h1
      class="text-black text-4xl flex justify-center uppercase font-outline-2 shadow-white font-snapa">
      Oro Chat Draft
    </h1>
    <div class="text-white text-4xl rounded-sm font-snapa">
      {#if data.hide != 'choice'}
        ...
        <div
          class="bg-purple-900 bg-opacity-70 text-white text-4xl rounded-t-lg"
          class:rounded-b-lg={data.hide != 'deck'}>
          <div class="flex items-center">
            <h2 class="font-outline pl-4">
              <span class="uppercase font-snapa font-outline shadow-black"
                >Pick:</span>
              <span class="font-snapn font-outline shadow-black"
                >{current_draft.total + 1}</span>
            </h2>
            <!-- Countdown Timer Icon & Seconds -->
            <div
              class="items-center inline-flex absolute right-4 font-snapn font-outline shadow-black">
              <iconify-icon
          ...
          <div
            ...
            {#if choices && choices.length > 0}
              {#each choices as choice, index}
                <div
                  class="border-white bg-black bg-opacity-70 border-2 rounded-lg shadow-md shadow-black/100 relative"
                  class:blur={winningCard &&
                    winningCard.cardDefKey != choice.cardDefKey}>
                  <!-- Selection Value-->
                  <span
                    class="font-bold text-8xl flex items-center absolute top-0 left-0 bottom-0 m-auto">
                    {index + 1}
                  </span>
                  ...
                  <div class="flex justify-center absolute bottom-0 right-2">
                    <div class="flex items-center">
                      <iconify-icon
                        ...
                        height="32"
                        flip="horizontal"
                        class="h-8 stroke-black stroke-1 fill-white drop-shadow-[-2px_2px_2px_var(--tw-shadow-color)] inline-block transform translate-x-1.5" />
      ...
      {#if data.hide != 'deck'}
        <!-- Drafted Cards -->
        <div
          class="bg-purple-900 bg-opacity-70 text-slate-200 text-2xl font-outline rounded-t-lg">
      ...
      {/if}
      <p
        class="bg-purple-900 bg-opacity-70 rounded-b-lg text-xl flex justify-center font-outline uppercase shadow-black font-snapa">
        To use Chat Draft, inquire at:&nbsp;<span
          class="anchor font-serif font-outline-0 normal-case relative bottom-[2px]"
          >twitch.tv/jjrolk</span>
      </p>
    </div>
  {:else if data.hide != 'deck'}
    ...
    <div
      class="flex flex-row bg-black/70 text-white bg-purple-900 bg-opacity-70 text-4xl rounded-t-lg">
    ...
    <p
      class="bg-purple-900 bg-opacity-70 rounded-b-lg text-xl flex justify-center font-outline uppercase shadow-black text-white font-snapa">
      To use Chat Draft, inquire at:&nbsp;<span
        class="anchor font-serif font-outline-0 normal-case relative bottom-[2px]"
        >twitch.tv/jjrolk</span>
    </p>
