      ...
      <Stepper buttonCompleteLabel="Finish" on:complete={onComplete}>
        ...
        <Step locked={!data.botInChannel}>
          ...
          {#if data.botInChannel}
            ...
            <br />
            <p>
              Recommendation: Provide chat draft bot with VIP status - type <code
                class="code">/vip chatdraftbot</code> into your Twitch chat. VIP
              status allows the bot to function with follower/subscriber only mode
              enabled.
            </p>
            <br />
          ...
          {:else}
            <p>
              The chat draft bot needs to be added to your Twitch channel. Click
              the “Join Channel” button to invite the bot.
            </p>
              ...
              use:enhance={() => {
                return async ({ result, update }) => {
                  if (result.type == 'success') {
                    toastStore.trigger({
                      message: 'Bot successfully joined your channel.',
                    });
                  }
                  update();
                };
