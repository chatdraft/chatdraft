...
import {
  GetPreviewStatus,
  TogglePreviewStatus,
} from '$lib/server/previewHandler';
...
import {
  DbAddChannel,
  DbRemoveChannel,
  DbResetUserCollection,
  DbUpdateUserCollection,
  ...
  updatePreferences: async ({ locals, request }) => {
    if (locals.user && locals.user.twitchID) {
      ...
      if (data) {
        ...
        const userPreferences = await DbUpdateUserPreferences(
  ...
  uploadCollection: async ({ locals, request }) => {
    if (locals.user && locals.user.twitchID) {
      ...
      if (collectionData) {
        ...
        if (cards) {
          const userPreference = await DbUpdateUserCollection(
            locals.user.twitchID,
            cards
          );
