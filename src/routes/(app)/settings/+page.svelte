...
<div class="p-4">
    ...
    {#if data.botInChannel}
        ...
        <form method="POST" action="?/part" use:enhance on:submit={invalidateAll}><button class="btn btn-lg variant-filled-warning m-4">Leave channel</button></form>
    {:else}
        Have ChatDraftBot join your channel:
        <form method="POST" action="?/join" use:enhance on:submit={invalidateAll}><button class="btn btn-lg variant-filled-primary m-4">Join channel</button></form>
    {/if}
    <br/>
