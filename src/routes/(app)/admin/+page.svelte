...
<section class="p-4">
    ...
    use:enhance={() => {
      return async ({ result, update }) => {
        if (result.type == 'success') {
          toastStore.trigger({
            message: 'Card database successfully updated.',
          });
        }
        update();
      };
  ...
  <form
    ...
    use:enhance={({ cancel }) => {
      if (
        !confirm(
          'Are you sure you want to reset the card database to installed default?'
        )
      ) {
        cancel();
      }
      ...
      return async ({ result, update }) => {
        if (result.type == 'success') {
          toastStore.trigger({
            message: 'Card database successfully reset to build settings.',
          });
        }
        update();
      };
